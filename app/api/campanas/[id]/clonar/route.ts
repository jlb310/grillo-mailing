...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const clone = await prisma.campaign.create({
    data: {
      ...
      logoRightUrl:      src.logoRightUrl,
      logoRightHeight:   src.logoRightHeight,
      logoRight2Url:     src.logoRight2Url,
      logoRight2Height:  src.logoRight2Height,
      headerColor:  src.headerColor,
      footerText:   src.footerText,
      blocks:       src.blocks ?? undefined,
      ctaButtons:   src.ctaButtons ?? undefined,
      products:     src.products ?? undefined,
      socials:      src.socials ?? undefined,
