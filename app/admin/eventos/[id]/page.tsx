...
import { ArrowLeft, Calendar, MapPin, Users, Plus, Mail, Clock } from "lucide-react";
  ...
  return (
    <div className="p-6 space-y-6 max-w-5xl">
      <div className="flex items-center justify-between">
        ...
        <Link href={`/admin/campanas/nueva?eventId=${event.id}`}>
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            <Calendar className="w-5 h-5 text-[#207029]" />
            <div>
        ...
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            <MapPin className="w-5 h-5 text-[#207029]" />
            <div>
        ...
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            <Clock className="w-5 h-5 text-[#207029]" />
            <div>
              <p className="text-xs text-gray-400">Horas lectivas</p>
              <p className="text-sm font-medium">{event.horasLectivas} hrs</p>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader className="pb-3 flex flex-row items-center justify-between">
            <CardTitle className="text-sm font-semibold flex items-center gap-2">
              <Mail className="w-4 h-4 text-[#207029]" /> Campañas ({event.campaigns.length})
            </CardTitle>
            <Link href={`/admin/campanas/nueva?eventId=${event.id}`}>
        ...
        <Card>
          <CardHeader className="pb-3 flex flex-row items-center justify-between">
            <CardTitle className="text-sm font-semibold flex items-center gap-2">
              <Users className="w-4 h-4 text-[#207029]" /> Contactos ({event.contacts.length})
            </CardTitle>
            <UploadContactosButton eventId={event.id} />
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="border-t pt-3">
              ...
              <GruposPanel
                eventId={event.id}
                groups={event.contactGroups}
                totalContacts={event.contacts.length}
                ungroupedCount={
                  event.contacts.length -
                  event.contactGroups.reduce((acc, g) => acc + g._count.contacts, 0)
                }
              />
            </div>
          </CardContent>
